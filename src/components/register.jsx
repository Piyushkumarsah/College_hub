...
const Chapter = () => {
  ...
  return (
    <div className="chapter mx-auto px-20 w-full h-screen bg-black p-4">
      <h2 className="text-orange-400 text-lg font-bold mb-4">Select Subject</h2>
      <div className="flex flex-wrap gap-4">
        {Object.keys(topics).map((subject) => (
          <div key={subject} className="flex flex-col">
            <button
              className={`bg-orange-500 text-white  px-4 py-2 font-semibold rounded mb-2 ${
      ...
      {selectedSubject && (
        <div className="mt-4">
          <h2 className="text-orange-400 text-lg font-bold">{selectedSubject}</h2>
              ...
              {Object.keys(topics[selectedSubject].chapters).map((chapter) => (
                <React.Fragment key={chapter}>
                  <tr className=" bg-black text-orange-400 ">
                    <td
                      className={`py-2 px-4 cursor-pointer bg-black text-orange-400 hover:bg-orange-600 hover:text-white ${
                  ...
                  {selectedChapter === chapter &&
                    topics[selectedSubject].chapters[chapter].map((topic) => (
                      <tr key={topic.name}>
                        <td className="py-2 px-4 bg-black text-white shadow-md shadow-orange-400 ">{topic.name}</td>
                        <td className="py-2 px-4 bg-black text-white shadow-md shadow-orange-400">
                          <a
                            href={topic.video}
                            target="_blank"
                            rel="noreferrer"
                            className="text-blue-500 hover:underline py-2 px-4  rounded-2xl shadow-md sm:shadow-orange-400"
                          >
                            Watch Video
                          </a>
                        </td>
